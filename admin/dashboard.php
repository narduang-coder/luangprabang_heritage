...
<body>
<div class="sidebar">
    <div class="p-3 text-center border-bottom border-success mb-3"><i class="fas fa-landmark fa-2x mb-2"></i><h6 class="mb-0 d-none d-md-block">ມໍລະດົກຫຼວງພະບາງ</h6></div>
    <nav class="nav flex-column">
        ...
        <a class="nav-link" href="add_user.php"><i class="fas fa-plus-circle"></i> <span>ເພີ່ມຜູ້ໃຊ້ງານ</span></a>
        <a class="nav-link" href="../api/logout.php"><i class="fas fa-sign-out-alt"></i> <span>ອອກຈາກລະບົບ</span></a>
    </nav>
</div>
<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-4"><h2><i class="fas fa-tachometer-alt text-success"></i> ໜ້າຫຼັກ</h2><span class="badge bg-success p-2"><i class="fas fa-user"></i> <?php echo $_SESSION['admin_name']; ?></span></div>
    <div class="row mb-4">
    ...
    <div class="card card-custom"><div class="card-header bg-white"><h5 class="mb-0"><i class="fas fa-list text-success"></i> ເຮືອນມໍລະດົກລ່າສຸດ</h5></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0"><thead class="table-light"><tr><th>QR Code</th><th>ຊື່ເຮືອນ (ລາວ)</th><th>ຊື່ເຮືອນ (ອັງກິດ)</th><th>ການຈັດການ</th></tr></thead><tbody>
<?php while ($house = mysqli_fetch_assoc($recentHouses)) { ?>
<tr><td><code><?php echo htmlspecialchars($house['qr_code']); ?></code></td><td><?php echo htmlspecialchars($house['house_name_lo'] ?: '-'); ?></td><td><?php echo htmlspecialchars($house['house_name_en'] ?: '-'); ?></td>
<td><a href="edit_house.php?id=<?php echo $house['house_id']; ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
<button onclick="deleteHouse(<?php echo $house['house_id']; ?>)" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button>
<button onclick="generateQR(<?php echo $house['house_id']; ?>)" class="btn btn-sm btn-info"><i class="fas fa-qrcode"></i></button></td></tr>
...
function deleteHouse(houseId) { 
    Swal.fire({ 
        ...
        if (result.isConfirmed) { 
            ...
            $.ajax({ 
                url: 'simple_delete_ajax.php',  // ປ່ຽນຈາກ 'api/delete_house.php' ເປັນ 'simple_delete_ajax.php'
                method: 'POST', 
...
function generateQR(houseId) { Swal.fire({ title: 'ກຳລັງສ້າງ QR Code...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } }); $.ajax({ url: 'generate_qr.php', method: 'POST', data: { house_id: houseId }, dataType: 'json', success: function(response) { Swal.close(); if (response.success) { Swal.fire({ title: 'ສ້າງ QR Code ສຳເລັດ', html: `<img src="../${response.qr_url}" style="width: 200px; height: 200px;"><br><code>${response.qr_id}</code><br><a href="../${response.qr_url}" download class="btn btn-success mt-2">ດາວໂຫຼດ QR Code</a>`, showConfirmButton: true }).then(() => location.reload()); } else { Swal.fire('ຜິດພາດ!', response.message, 'error'); } }, error: function() { Swal.close(); Swal.fire('ຜິດພາດ!', 'ບໍ່ສາມາດສ້າງ QR Code ໄດ້', 'error'); } }); }
</script>
